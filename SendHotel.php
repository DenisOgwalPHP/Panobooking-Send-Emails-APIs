...
$messsage = "Esteemed Service Provider, We wish to inform you that a booking for your facility has been made by our Clients Community. Please check your Panobooking Dashboard to confirm. or Reach out to 0741822377 for further details";

require_once('connect.php');
$sales6 ="SELECT ContactNo FROM `pickuplocations` where EmailAddress='".$to."'";
$result5=mysqli_query($link,$sales6);
$counter=1; $numbers='';
while ($row5 = mysqli_fetch_assoc($result5)) {
	$number = trim($row5['ContactNo']);
	if ($counter == 1) {
		$numbers = $number;
	} else {
		$numbers .= ',' . $number;
